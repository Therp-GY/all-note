...
- 创建模型
  ...
  在myapp/model下靠python语言定义一个个class，它们就是我们数据库中的模型（定义表啊字段啊什么的）

- 激活模型

  在  项目/settings.py 中的配置类 [`INSTALLED_APPS`](https://docs.djangoproject.com/zh-hans/2.1/ref/settings/#std:setting-INSTALLED_APPS) 中添加设置，加入我们的项目，然后运行 python manage.py makemigrations polls，对我们模型文件进行修改（你对模型的新的修改，和一些必要字段的加入），然后运行python manage.py migrate指令即可在数据库中创建新定义的模型的数据表。
...
- 将表加入管理
  ...
  在项目的admin.py中添加admin.site.register(某个类)即可将这个类添加到admin的管理中

## 视图

视图的加载实际上就是 应用/urls.py 调用 应用/views.py中的各个方法，

同时应用/urls.py后可追加后缀url来加载不同的视图。

 应用/views.py中的方法可以传入和myapp同级的templates中的html等文件中来加载视图
...
urlpatterns = [
    ...
    # ！！！ <>向方法中传入参数，参数的传递就是传递的url通过正则剪切然后传入
    # ex: /polls/5/results/
...
def hello(request):
	...
	# 在和myapp应用同级的template中查找html并传入参数完成视图加载
