...
- 步骤
  ...
  - 卷积convolution
    ...
    **【若为RGB图像，则一个filter为一个3 * （3 * 3）立方体，卷积时每个3 * （3 * 3）的filter和3 * 4 * 4（三通道 * 长 * 宽）图像卷积，或者说看成3（通道数）个1  * （3 * 3）的filter 分别卷积 3（通道数）张 1 *（4 * 4）的图片，这时候每个卷积都为单通道（分别为R,G,B通道），卷积出3张再堆叠为一张feature map 】**

    **【注意不论几次的convolution，每次convolution后都是得到filter个matrix，无非是matrix的维数可能在不断升高（但每次计算算内积，因此无碍）**

    如:  

    1. 25 * (3 * 3) filter 卷积  1 * (28 * 28) --> 25 * (26 * 26) feature map
    2. 2 * 2 max pooling   --> 25 * (13 * 13)
    3. 50 * (3 * 3) filter 卷积 25 * (13 * 13)  --> 50 * (11 * 11) feature map   注意这里的filter不是一个3 * 3 的matrix，而是一个 25 * 3 * 3的立方体，因为原图像每个像素有25个通道，相应的feature map实际上也不是(11 * 11)的matrix而是 25 * 11 * 11
    4. 2 * 2 max pooling
...
多通道卷积：https://zhuanlan.zhihu.com/p/251068800
...
- yolov3
  ...
  有3个预测特征层 N * N * (3 * (4+1+80))  以coco为例，每个预测特征层有多个anchor先验框
...
python -m torch.distributed.launch --nproc_per_node 2 train.py --data data/lesion.yml  --cfg yolov3-tiny-GG.yaml  --weights yolov3-tiny.pt --device 3,4  --name GY-yolov3-tiny
